/* Segment Tree - I16 Small Tier (2-TIER SYSTEM)
 * Auto-generated from template. DO NOT EDIT MANUALLY.
 */

#include "segmentI16.h"

#define SEGMENT_SUFFIX I16
#define SEGMENT_VALUE_TYPE int16_t
#define SEGMENT_INDEX_TYPE_SMALL uint32_t
#define SEGMENT_INDEX_TYPE_FULL uint64_t
#define SEGMENT_SMALL_MAX_COUNT (32 * 1024)
#define SEGMENT_SMALL_MAX_BYTES (256 * 1024)
#define SEGMENT_TYPE_MIN INT16_MIN
#define SEGMENT_TYPE_MAX INT16_MAX
#define SEGMENT_IS_SIGNED 1
#define SEGMENT_IS_FLOATING 0
#define SEGMENT_IMPL_SCOPE

#include "segmentCoreImpl.h"
